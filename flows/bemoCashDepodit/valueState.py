...
async def get_value(update: Update, context: CallbackContext) -> int:
    ...
    if validate(value):
        ...
        user_id = user.get('id')
        BemoTransaction().insert({'transfeer_num' : transfeer_num , 'user_id': user_id , 'status':'pending','action_type':'deposit' , 'value' : value})
       
       
        transfeer = BemoTransaction().getBy({'transfeer_num' : ('=', transfeer_num)})[0]

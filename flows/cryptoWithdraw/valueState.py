    ...
    if db:
        ...
        if validate(value):
            ...
            if balanceValidate(value , balance , currency_name):
                ...
                CryptoTransaction(cursor).insert({'transfeer_num' : transfeer_num , 'user_id': user_id , 'status':'pending','action_type':'withdraw' , 'value' : -1*float(value)*float(config.crypto.SYP_for_unit[currency_name]) , 'currency_name' : currency_name , 'network_name' : network_name , 'SYP_for_unit' : config.crypto.SYP_for_unit[currency_name]})

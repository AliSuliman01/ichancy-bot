...
from flows.bemoWithdrawal.validation.valueValidation import balanceValidate , vlueValidate
from config.bemo import TAX , BEMO_WITHDRAW_GROUP
     ...
     if db:
          ...
          if vlueValidate(value):
               ...
               if balanceValidate(value , balance):
                    chat_id = BEMO_WITHDRAW_GROUP
                    user_id = user.get('id') 
                    withdraw_number = context.user_data['withdraw_number']
                    BemoTransaction(cursor).insert({'transfeer_num' : withdraw_number , 'user_id': user_id , 'status':'pending','action_type':'withdraw' , 'value' : -value})
                    transfeer_id = cursor.lastrowid
                    transfeer = BemoTransaction(cursor).getById(transfeer_id)
                    provider_type = "bemo"

   ...
   try:
    if db:
        ...
        if validate(value):
            ...
            CryptoTransaction(cursor).insert({'transfer_num' : transfer_num , 'user_id': user_id , 'status':'pending','action_type':'deposit' , 'value' : float(value)*float(config.crypto.SYP_for_unit[currency_name]) , 'currency_name' : currency_name , 'network_name' : network_name , 'SYP_for_unit' : config.crypto.SYP_for_unit[currency_name]})

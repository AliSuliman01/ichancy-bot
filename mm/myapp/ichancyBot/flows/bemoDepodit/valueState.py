...
from config.bemo import BEMO_DEPOSIT_GROUP
...
async def get_value(update: Update, context: CallbackContext) -> int:
   ...
   try:
    if db:
        ...
        chat_id = BEMO_DEPOSIT_GROUP
        if validate(value):
            transfer_num = context.user_data['transfer_num']
            telegram_id = update.message.from_user.id
            user = User(cursor).getBy({'telegram_id':('=', telegram_id)})[0]
            user_id = user.get('id')
            BemoTransaction(cursor).insert({'transfer_num' : transfer_num , 'user_id': user_id , 'status':'pending','action_type':'deposit' , 'value' : value})
